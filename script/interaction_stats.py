...
ax.bar(list(range(len(hist[0]))), hist[0])
...
ax.set_xticklabels(gene_groups.keys())
ax.grid(axis='x')
...
ax = sns.distplot(np.log10(interactions.groupby('Geneid')['WT mRNA expression'].first()), color='black', ax=ax)
