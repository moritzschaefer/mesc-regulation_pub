...
if snakemake.params['plot_upregulation_predictions']:
    ...
    up_genes_filtered_deg = diffexp_df.loc[up_genes_filtered, 'log2FoldChange']
    sns.ecdfplot(up_genes_filtered_deg, ax=ax2,
                 label=f'filtered RNAi_KO-upregs (n={len(up_genes_filtered)})', color='#60cda5')

    print(f'Mean filtered upreg DEG: {up_genes_filtered_deg.mean()}')
    print(f'Kolmogorov-Smirnov test between DEGs of {len(up_genes_filtered)} "commonly upregulated genes" filtered for miR-291/294/295 binding sites (seed match AAGUGCU) vs the predictions from the paper\'s integrative approach: {kstest(up_genes_filtered_deg, target_deg)}')
...
print(f'Mean for interaction-score-filtered targets: {score_filtered_deg.mean()}')
print(f'Kolmogorov-Smirnov test between DEGs of {len(score_filtered)} Interaction-score-filtered targets vs the predictions from the paper\'s integrative approach: {kstest(score_filtered_deg, target_deg)}')
...
sns.ecdfplot(low_up_genes_deg, ax=ax3, label=f'lowly upregulated genes (n={len(low_up_genes)})', color='#7ec0ee')
...
print(f'Kolmogorov-Smirnov test between DEGs of {len(score_filtered)} low-up-genes vs filtered low-up-genes: {kstest(low_up_genes_deg, low_target_deg)}')
